...
async function main(){
 ...
 try{
  ...
  await mutateMenu(actor,{action:"reorder",entity:"item",categoryId,ids:[nativeId,importedId]});const ordered=await sql`SELECT id FROM ordering_menu_items WHERE id IN(${nativeId},${importedId}) ORDER BY sort_order`;if(String(ordered[0]?.id)!==nativeId||String(ordered[1]?.id)!==importedId)throw new Error("Item drag order was not persisted.");
  ...
  console.log(JSON.stringify({stableImportedId:true,localPriceSurvivesImport:true,resetToImported:true,nativeItemSurvivesImport:true,nativeStableId:nativeId,dragOrderPersisted:true,historicalPriceSnapshot:true,modifierMinMax:true,crossBusinessRejected:true,audited:true,integerCents:true},null,2));
 } finally {
