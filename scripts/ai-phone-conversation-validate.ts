...
async function main(){
  ...
  for(const phrase of ["2–10 spoken words","Capture names, phones, addresses","If interrupted, stop immediately","SHADOW","Never say the order is placed","Maximum"]){assert.ok(prompt.includes(phrase),`Missing prompt policy: ${phrase}`)}
