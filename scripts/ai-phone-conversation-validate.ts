...
async function main(){
  ...
  assert.ok(prompt.includes("Turkey Big Boss item with Full Sub")&&prompt.includes("never reject them or substitute another sandwich"),"Turkey sub business aliases must remain authoritative.");
  ...
  assert.ok(prompt.includes("This line is only for Corner Deli orders and restaurant questions. Please call back if you need to place an order. Goodbye.")&&prompt.includes("Do not apply this rule to a misunderstood food name"),"Off-topic callers must receive the bounded warning without blocking legitimate restaurant requests.");
  assert.ok(sidebandSource.includes("this line is only for corner deli orders and restaurant questions")&&sidebandSource.includes("hangupAfterPlayback=true"),"Off-topic warning playback must trigger a deterministic hangup.");
  assert.ok(prompt.includes("Mayo, Russian, oil, or shakers?")&&prompt.includes("Lettuce, tomato, onions, or hot peppers?")&&prompt.includes("American, Swiss, or provolone?")&&prompt.includes("never ask for cheese first")&&prompt.includes("Do not run this checklist for any Big Boss item"),"Regular sub build questions and the Big Boss exclusion must remain authoritative.");
  assert.ok(prompt.includes("Do you mean a Nacho Supreme?")&&prompt.includes("Do you want everything on that?")&&prompt.includes("Buffalo Grilled Chicken Salad"),"Nacho Supreme and chicken-salad clarification rules must remain authoritative.");
