...
async function main(){
  ...
  for(const fixture of fixtures){await recordAiRegression({business:"Corner Deli",caseType:"order_resolution",source:"permanent_fixture",payload:{serviceType:"pickup",items:fixture.items},expected:{items:fixture.expectedItems||[],modifier:fixture.expectedModifier||null,errorCode:fixture.errorCode||null}})}
  ...
  let passed=0;for(const row of stored){const payload=row.input as {serviceType?:string;items?:any[]};if(!Array.isArray(payload.items)||!payload.items.length)continue;let priced:Record<string,any>|undefined;try{priced=await priceSpokenOrder({business:"Corner Deli",actor,service:"pickup",items:payload.items});assert.ok(!row.expected?.errorCode,`${row.source}: expected ${row.expected.errorCode} but order was created`);const names=priced.lines.map((line:Record<string,any>)=>line.item_name_snapshot);for(const expected of (row.expected?.items||[]))assert.ok(names.includes(expected),`${row.source}: missing ${expected}`);if(row.expected?.modifier){const modifiers=await sql`SELECT option_name_snapshot FROM ordering_order_item_modifiers WHERE order_item_id IN(SELECT id FROM ordering_order_items WHERE order_id=${priced.id})`;assert.ok(modifiers.some((modifier:Record<string,any>)=>modifier.option_name_snapshot===row.expected.modifier),`${row.source}: missing ${row.expected.modifier}`)}}catch(error){if(!row.expected?.errorCode)throw error;assert.ok(error instanceof AiToolError);assert.equal(error.code,row.expected.errorCode)}finally{if(priced?.id)await sql`DELETE FROM ordering_orders WHERE id=${priced.id}`}passed++}
