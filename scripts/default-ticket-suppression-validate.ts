...
void (async()=>{
  ...
  const option="Mayonnaise";const cases=[
    ...
    [{option_name_snapshot:option,quantity:1,selection_state:"selected",amount:"light" as const,print_on_ticket:true},"LIGHT MAYONNAISE"],
    [{option_name_snapshot:option,quantity:1,selection_state:"selected",amount:"heavy" as const,print_on_ticket:true},"HEAVY MAYONNAISE"],
  ...
  console.log(JSON.stringify({defaultNormalSuppressed:true,light:"LIGHT MAYONNAISE",heavy:"HEAVY MAYONNAISE",removed:"NO MAYONNAISE",singleSelectReplacementSuppressesOldDefault:true,realPizzaDefaults:defaults,pizzaToppingFormatterPreserved:true},null,2));
  process.exit();
