...
assert.doesNotMatch(agi,/pocketsphinx|debug_recognition|VOICE_PAYMENT_SANDBOX_RECOGNITION/);
...
assert.match(agi,/card=enter_card_number\(\)/);
assert.match(agi,/def valid_luhn\(value\)/);
assert.match(agi,/not valid_luhn\(card\)/);
assert.match(agi,/def enter_expiration\(\)/);
assert.match(agi,/1<=int\(value\[:2\]\)<=12/);
assert.match(agi,/keypad\("dtmf-confirm",10000,1\)=="1"/);
assert.match(agi,/keypad\("dtmf-security-code",12000,4\)/);
assert.match(agi,/keypad\("dtmf-billing-zip",12000,5\)/);
...
assert.match(lib,/await assertVoiceOrderReady\(String\(call\.order_id\)\)/);
assert.match(lib,/await assertVoiceOrderReady\(String\(session\.order_id\)\)/);
assert.match(lib,/status='approved_unapplied'/);
assert.match(lib,/failure_code='order_commit_failed'/);
