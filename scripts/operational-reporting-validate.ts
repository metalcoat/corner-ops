...
void (async()=>{
  ...
  const today=new Intl.DateTimeFormat("en-CA",{timeZone:"America/New_York",year:"numeric",month:"2-digit",day:"2-digit"}).format(new Date(Date.now()-4*60*60*1000));
  try {
    const before=await orderingOperationalReport({business:'Corner Deli',start:today,end:today});
    ...
    if(Number(report.summary.orders)-Number(before.summary.orders)!==1||Number(report.summary.gross_merchandise_cents)-Number(before.summary.gross_merchandise_cents)!==1000||Number(report.summary.order_total_cents)-Number(before.summary.order_total_cents)!==1350)throw new Error('Finalized sales snapshot aggregation failed.');
    if(report.salesByServiceType.some(row=>row.label==='bar'))throw new Error('Cross-business report isolation failed.');
    const cash=report.tenders.find(row=>row.tender_type==='cash'),beforeCash=before.tenders.find(row=>row.tender_type==='cash');
    if(Number(cash?.payments_cents||0)-Number(beforeCash?.payments_cents||0)!==1350||Number(cash?.reversals_cents||0)-Number(beforeCash?.reversals_cents||0)!==250)throw new Error('Tender/reversal aggregation failed.');
