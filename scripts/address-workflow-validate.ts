...
const fetcher = async (input: string | URL | Request, init?: RequestInit) => {
  ...
  if (url.includes("places:autocomplete")){autocompleteBody=JSON.parse(String(init?.body||"{}"));return Response.json({ suggestions: [{ placePrediction: { placeId: "place-1", text: { text: "12 Main St, Testville, NY" }, structuredFormat: { mainText: { text: "12 Main St" }, secondaryText: { text: "Testville, NY" } } } }] });}
...
async function main() {
  ...
  if(!autocompleteBody?.locationRestriction?.circle||autocompleteBody.locationBias||Math.abs(Number(autocompleteBody.locationRestriction.circle.radius)-19312.128)>.01)throw new Error("Autocomplete is not restricted to the 12-mile delivery area.");
  ...
  let outsideRejected=false;try{await routeDeliveryAddress(address,{env,fetcher:(async()=>Response.json({routes:[{distanceMeters:20921.472,duration:"1200s"}]})) as typeof fetch})}catch(error){outsideRejected=String(error).includes("outside the 12-mile")}if(!outsideRejected)throw new Error("Address beyond 12 miles was accepted.");
  ...
  console.log(JSON.stringify({ inputNormalization: true, shortInput: true, providerFailure: true, providerResponseNormalization: true, autocompleteRestrictedTo12Miles:true,selectedAddressValidation: true, changedAddressInvalidatesValidation: true, deliveryGuard: true, pickupWithoutAddress: true, dineInWithoutAddress: true, apiKeyNotExposed: true, drivingRoute: true,outsideRadiusRejected:true }, null, 2));
}
