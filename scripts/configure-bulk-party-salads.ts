...
async function main(){
  ...
  await withTransaction(async()=>{const sql=getSql();
    await sql`INSERT INTO ordering_menu_categories(id,business,name,display_name,parent_id,sort_order,active,presentation_only) VALUES(${CATEGORY},'Corner Deli','Salads & Deli / Party Salads','Bulk / Party Salads',${PARENT},290,TRUE,FALSE) ON CONFLICT(id) DO UPDATE SET name='Salads & Deli / Party Salads',display_name='Bulk / Party Salads',parent_id=${PARENT},sort_order=290,active=TRUE`;
    await sql`INSERT INTO ordering_category_overrides(category_id,display_name,parent_id,parent_id_overridden,sort_order,visible,updated_by) VALUES(${CATEGORY},'Bulk / Party Salads',${PARENT},TRUE,290,TRUE,'bulk-party-salads') ON CONFLICT(category_id) DO UPDATE SET display_name='Bulk / Party Salads',parent_id=${PARENT},parent_id_overridden=TRUE,sort_order=290,visible=TRUE,updated_by='bulk-party-salads',updated_at=NOW()`;
    ...
    await sql`UPDATE ordering_menu_categories SET active=FALSE,updated_at=NOW() WHERE business='Corner Deli' AND name='Salads & Deli / Bulk' AND NOT EXISTS(SELECT 1 FROM ordering_menu_items WHERE category_id=ordering_menu_categories.id) AND NOT EXISTS(SELECT 1 FROM ordering_menu_categories child WHERE child.parent_id=ordering_menu_categories.id AND child.active=TRUE)`;
  });
