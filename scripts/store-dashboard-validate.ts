...
async function main(){
  ...
  try{
    ...
    console.log(JSON.stringify({timedOrder:dashboard.timedOrders.some(row=>row.display_number===display),deliveryCreated:Boolean(delivery),unassignedTask:dashboard.tasks.find(task=>task.key==="unassigned")!.count>0,unpaidTask:dashboard.tasks.find(task=>task.key==="unpaid")!.count>0},null,2));
    if(!delivery||!dashboard.timedOrders.some(row=>row.display_number===display))throw new Error("Store dashboard did not surface the timed delivery fixture.");
