...
async function main(){
  const [{getSql},{ensureOrderingSchema},{activeDeliCalls,acknowledgeDeliCall,callerFromLivePosFeed,claimCallerFromAiIngress,ingestThreeCxLiveCall}]=await Promise.all([import("../src/lib/db"),import("../src/lib/ordering-db"),import("../src/lib/three-cx-live-calls")]);
  await ensureOrderingSchema();
  const sql=getSql(),customerId=randomUUID(),phoneId=randomUUID(),orderId=randomUUID(),callId=`validation-${randomUUID()}`,aiCallId=`ai-validation-${randomUUID()}`,actorId=randomUUID(),caller="3155550188",aiCaller="3155550199",queue=process.env.THREE_CX_DELI_QUEUE||"90";
  try{
    ...
    await ingestThreeCxLiveCall({callId:aiCallId,callerNumber:aiCaller,status:"ringing",source:"ai_ingress"});
    const aiHiddenFromPos=!(await activeDeliCalls()).some(row=>String(row.call_id)===aiCallId),claimedAiCaller=await claimCallerFromAiIngress(),aiClaimIsSingleUse=(await claimCallerFromAiIngress())==="";
    ...
    const result={accepted:accepted.accepted===true,customerMatched:String(match?.customer_id||"")===customerId,openOrderMatched:String(match?.open_order_id||"")===orderId,lineMatched:String(match?.line_number||"")==="2",sharedCallerMatched:sharedCaller===caller,aiCallerClaimed:claimedAiCaller===aiCaller,aiClaimIsSingleUse,aiHiddenFromPos,acknowledged:acknowledged&&!remaining,otherQueueIgnored:ignored.accepted===false};
    console.log(JSON.stringify(result,null,2));
    if(Object.values(result).some(value=>!value))throw new Error("3CX live-call validation failed.");
  }finally{
    await sql`DELETE FROM three_cx_live_calls WHERE call_id IN(${callId},${aiCallId},${`ignored-${callId}`})`;
