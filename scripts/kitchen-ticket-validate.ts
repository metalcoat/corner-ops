...
const delivery = printPayloadLines({
  ...
  orderInstructions: "Please ring the side-door bell",
  timingLines: deliveryTiming,
  paymentLabel: "AMOUNT DUE: $24.75",
  lines: ["1 X PIZZA"],
});
const paid = printPayloadLines({
  heading: "KITCHEN ORDER",
  orderNumber: "1285",
  customerName: "Paid Guest",
  phone: "315-555-1213",
  serviceType: "pickup",
  timingLines: pickupTiming,
  paymentLabel: "PAID",
  lines: ["1 X SUB"],
});
...
for (const expected of [
  ...
  "PHONE: 315-555-1212",
  "TYPE: DELIVERY",
  "DELIVER TO: 1121 Paterson Street, Ogdensburg, NY 13669",
  "DROP-OFF: Lane 14",
  ...
  if (!delivery.includes(expected))
    throw new Error(`Delivery ticket is missing: ${expected}`);
}
if (!paid.includes("PAID") || paid.some((line) => line.includes("AMOUNT DUE")))
