...
async function main() {
  ...
  try {
    await withTransaction(async () => {
      ...
      const final = await commitTender({ orderId, business: "Corner Deli", tenderType: "card", amountTenderedCents: 2200, clientMutationId: "payment-test-card", actor, providerApproval: { provider: "helcim", transactionReference: `payment-test-${orderId}` } });
