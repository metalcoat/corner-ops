...
const diagnosticPath = new URL("../public/_migration-status.json", import.meta.url);
let activeStep = "startup";

async function writeDiagnostic(status) {
...
async function migrate() {
  ...
  const summary = {
    ...
    recipientSnapshots: Number(result?.recipient_snapshots || 0),
  };
  await writeDiagnostic(summary);
  console.log(`Database migrations complete: ${summary.messages} messages and ${summary.recipientSnapshots} recipient snapshots ready.`);
}
...
try {
  ...
  console.error(`Database migration failed during “${activeStep}”:`, error);
  await writeDiagnostic({ success: false, step: activeStep, error: message }).catch(() => undefined);
  if (force) throw error;
  // Diagnostic production build only: let Next.js deploy the status file so the
  // exact database failure can be read without exposing database credentials.
  console.error("Continuing this production build only to publish the sanitized migration diagnostic.");
}
