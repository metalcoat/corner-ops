...
type ComboRow = {
  ...
  combo_name: string;
  combo_prompt: string;
  combo_base_price_delta_cents: number;
  combo_sort: number;
  ...
  option_id: string | null;
  option_name: string | null;
  option_menu_item_id: string | null;
  option_price_delta_cents: number | null;
  option_available: boolean | null;
  option_sort: number | null;
};

export type OrderingModifierOptionView = {
  id: string;
  name: string;
...
export type OrderingModifierGroupView = {
  id: string;
  name: string;
  prompt: string;
  ...
  options: OrderingModifierOptionView[];
};

export type OrderingComboOptionView = {
  id: string;
  name: string;
...
export type OrderingComboGroupView = {
  id: string;
  name: string;
  prompt: string;
  minSelections: number;
  maxSelections: number;
  options: OrderingComboOptionView[];
};

export type OrderingComboView = {
  id: string;
  name: string;
  prompt: string;
  basePriceDeltaCents: number;
  groups: OrderingComboGroupView[];
};

export type OrderingMenuItemView = {
  id: string;
  categoryId: string;
  name: string;
  description: string;
  ...
  taxable: boolean;
  available: boolean;
  modifiers: OrderingModifierGroupView[];
  combos: OrderingComboView[];
};

export type OrderingMenuCategoryView = {
  id: string;
  name: string;
  items: OrderingMenuItemView[];
};
...
export async function orderingMenu(business: OrderingBusiness): Promise<OrderingMenuCategoryView[]> {
  ...
  const items = (await sql`
    SELECT id, category_id, name, description, sku, base_price_cents, taxable, available, sort_order
  ...
  const combos = (await sql`
    SELECT
      ...
      combo.prompt AS combo_prompt,
      combo.base_price_delta_cents AS combo_base_price_delta_cents,
      item_combo.sort_order AS combo_sort,
      grp.id AS group_id,
      grp.name AS group_name,
      grp.prompt AS group_prompt,
      grp.min_selections,
      grp.max_selections,
      grp.sort_order AS group_sort,
      opt.id AS option_id,
      opt.name AS option_name,
      opt.menu_item_id AS option_menu_item_id,
  ...
  for (const row of modifiers) {
    ...
    if (!group) {
      group = {
        ...
        allowOptionQuantity: Boolean(row.allow_option_quantity),
        options: [],
      };
    ...
    if (row.option_id && row.option_name) {
      group.options.push({
        ...
        available: Boolean(row.option_available),
        defaultSelected: Boolean(row.default_selected),
        includedQuantity: Number(row.included_quantity ?? 0),
      });
    }
  }
  ...
  for (const row of combos) {
    ...
    if (!combo) {
      combo = {
        id: row.combo_id,
        name: row.combo_name,
        prompt: row.combo_prompt,
        basePriceDeltaCents: Number(row.combo_base_price_delta_cents),
        groups: [],
      };
      comboMap.set(comboKey, combo);
      item.combos.push(combo);
    }
    ...
    if (!group) {
      ...
      comboGroupMap.set(groupKey, group);
    ...
    if (row.option_id && row.option_name) {
      group.options.push({
        id: row.option_id,
        name: row.option_name,
        menuItemId: row.option_menu_item_id,
        priceDeltaCents: Number(row.option_price_delta_cents ?? 0),
        available: Boolean(row.option_available),
      });
    }
  }
  ...
  return categories.map((category) => ({
    ...
    items: items
      ...
      .filter((item): item is OrderingMenuItemView => Boolean(item)),
  }));
}
