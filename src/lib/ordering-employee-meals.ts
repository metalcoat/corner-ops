...
export async function recordEmployeeMeal(input:{business:"Corner Deli";items:ConfiguredOrderItemInput[];note:string;breakAcknowledged:boolean;actor:OrderingActor}) {
  ...
  try {
    ...
    if(Number(priced.line_total_cents)>1000)throw new Error(`Employee meals are limited to $10.00 at customer menu pricing. This selection is $${(Number(priced.line_total_cents)/100).toFixed(2)}.`);
    ...
    await sql`UPDATE ordering_orders SET discount_cents=subtotal_cents+tax_cents,total_cents=0,paid_cents=0,amount_due_cents=0,payment_status='paid',updated_at=NOW() WHERE id=${order.id}`;
    await sql`UPDATE ordering_print_jobs SET payload=jsonb_set(jsonb_set(payload,'{heading}',to_jsonb('EMPLOYEE MEAL'::text)),'{paymentLabel}',to_jsonb('NO CHARGE · BREAK ONLY'::text)) WHERE order_id=${order.id} AND purpose='kitchen_production'`;
    await sql`INSERT INTO ordering_operations_audit(id,business,actor_id,actor_role,action,target_type,target_id,reason,details) VALUES(${randomUUID()},${input.business},${input.actor.id},${input.actor.role||"employee"},'employee_meal_recorded','employee_meal',${mealId},'',${JSON.stringify({employeeName:input.actor.name,orderId:order.id,customerPriceCents:Number(priced.line_total_cents),unmappedItems})}::jsonb)`;
    await dispatchSubmittedOrderPrintJobs(String(order.id),input.business);
    return{mealId,orderId:order.id,displayNumber:order.display_number,employeeName:input.actor.name,unmappedItems};
