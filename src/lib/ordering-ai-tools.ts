...
export type AiToolErrorCode = "INVALID_INPUT"|"ITEM_NOT_ON_MENU"|"INVALID_MODIFIER"|"INVALID_VARIANT"|"CATALOG_UNAVAILABLE"|"FULFILLMENT_REQUIRED"|"NOT_FOUND"|"VERSION_CONFLICT"|"VALIDATION_REQUIRED"|"NOT_AUTHORIZED"|"NOT_AVAILABLE"|"SEND_BLOCKED"|"INTERNAL_ERROR";
...
export type AiItemInput = VariantConfiguredOrderItemInput & {quantity:number};
...
export const spokenKey=(value:string)=>value.toLocaleLowerCase().replace(/&/g," and ").replace(/[^a-z0-9]+/g," ").trim().replace(/s\b/g,"");
export const BUSINESS_ITEM_ALIASES:Record<string,string[]>={pizza:["pie"],turkey:["turkey sub","regular turkey sub"],"turkey big bos":["turkey big boss sub","big boss turkey","big boss turkey sub"],wing:["wing","bone in wing","bone in wings","chicken wings"],"boneles wing":["boneless wing","boneless wings"],"2l pepsi":["2 liter pepsi","2 litre pepsi","two liter pepsi","two litre pepsi","2 l pepsi"],"2l diet pepsi":["2 liter diet pepsi","2 litre diet pepsi","two liter diet pepsi","two litre diet pepsi","2 l diet pepsi"],"large french frie":["large fry","large fries","large regular fries"],"small french frie":["small fry","small fries"],"buffalo chip":["buffalo chip","chips"]};
const itemAliases=BUSINESS_ITEM_ALIASES;
const editDistance=(a:string,b:string)=>{const row=Array.from({length:b.length+1},(_,i)=>i);for(let i=1;i<=a.length;i++){let prior=row[0];row[0]=i;for(let j=1;j<=b.length;j++){const next=Math.min(row[j]+1,row[j-1]+1,prior+(a[i-1]===b[j-1]?0:1));prior=row[j];row[j]=next}}return row[b.length]};
const similarity=(a:string,b:string)=>{if(a===b)return 1;if(a.includes(b)||b.includes(a))return .92-Math.abs(a.length-b.length)/Math.max(a.length,b.length)*.12;return 1-editDistance(a,b)/Math.max(a.length,b.length,1)};
function pizzaVariantAlias(value:string){const key=spokenKey(value);if(!key)return undefined;if(key.includes("sheet"))return"sheet pizza";if(key.includes("thick"))return"jumbo thick";if(key.includes("jumbo")||key.includes("large")||/\b16\b/.test(key))return"jumbo thin";if(key.includes("regular")||key.includes("medium")||/\b14\b/.test(key))return"regular";if(key.includes("small")||/\b12\b/.test(key))return"small";return undefined}
function subVariantAlias(value:string,defaultFull=false){const key=spokenKey(value);if(key.includes("wrap"))return"wrap";if(key.includes("half")||key.includes("1 2")||key.includes("small"))return"1 2 sub";if(key.includes("full")||key.includes("whole")||key.includes("large"))return"full sub";return defaultFull?"full sub":undefined}
function strictMatch<T>(query:string,rows:T[],terms:(row:T)=>string[],code:"ITEM_NOT_ON_MENU"|"INVALID_MODIFIER"|"INVALID_VARIANT",kind:string){const wanted=spokenKey(query),indexed=rows.map(row=>({row,terms:terms(row).map(spokenKey)})),exact=indexed.filter(candidate=>candidate.terms.includes(wanted));if(exact.length===1)return exact[0].row;if(exact.length>1)throw new AiToolError(code,`${kind} is ambiguous: ${query}.`,"Ask one short clarification question.",409);const ranked=indexed.map(candidate=>({...candidate,score:Math.max(...candidate.terms.map(term=>similarity(wanted,term)))})).sort((a,b)=>b.score-a.score),suggestion=ranked[0]&&ranked[0].score>=.72?terms(ranked[0].row)[0]:undefined;throw new AiToolError(code,code==="ITEM_NOT_ON_MENU"?"Sorry, we don't have that on the menu.":code==="INVALID_MODIFIER"?`Sorry, ${query} isn't an option on that.`:`Sorry, ${query} isn't an available size or option.`,suggestion?`You may offer ${suggestion}, but do not add it without confirmation.`:"Do not add it.",404,suggestion?{suggestion}:{});}
const spokenCatalogCache=new Map<OrderingBusiness,{expiresAt:number,value:Awaited<ReturnType<typeof orderingMenuWithVariants>>}>();
async function indexedSpokenCatalog(business:OrderingBusiness){const cached=spokenCatalogCache.get(business);if(cached&&cached.expiresAt>Date.now())return cached.value;try{const value=await orderingMenuWithVariants(business,"pos");spokenCatalogCache.set(business,{expiresAt:Date.now()+30_000,value});return value}catch{throw new AiToolError("CATALOG_UNAVAILABLE","I can't verify that item right now.","Do not add or improvise any item until the catalog is available.",503)}}
async function validateAiCatalogIds(business:OrderingBusiness,items:AiItemInput[]){const catalog=await indexedSpokenCatalog(business),all=catalog.flatMap(category=>category.items);for(const requested of items){if(!Number.isInteger(requested.quantity)||requested.quantity<1||requested.quantity>99)throw new AiToolError("INVALID_INPUT","Item quantity must be between 1 and 99.","Ask for a valid quantity.",409);const item=all.find(row=>row.id===requested.itemId&&row.available);if(!item)throw new AiToolError("ITEM_NOT_ON_MENU","The product ID is not on the active menu.","Do not add it.",404);if(requested.variantId&&!item.variants.some(row=>row.id===requested.variantId))throw new AiToolError("INVALID_VARIANT","The size or variant is not valid for that item.","Ask for a valid size.",409);if(!requested.variantId&&item.variants.length>1&&!item.variants.some(row=>row.defaultVariant))throw new AiToolError("INVALID_VARIANT","A required size or variant is missing.","Ask for a valid size.",409);const groups=new Map(item.modifiers.map(group=>[group.id,group])),options=new Map(item.modifiers.flatMap(group=>group.options.filter(option=>option.available).map(option=>[option.id,{group,option}] as const)));for(const[groupId,optionIds]of Object.entries(requested.modifierSelections||{})){const group=groups.get(groupId);if(!group||optionIds.some(id=>options.get(id)?.group.id!==groupId))throw new AiToolError("INVALID_MODIFIER","A modifier is not assigned to that product.","Do not convert it to a note.",409)}for(const group of item.modifiers){const count=(requested.modifierSelections?.[group.id]||[]).length;if(group.presentationContext==="ordinary"&&count<group.minSelections)throw new AiToolError("INVALID_MODIFIER",`${group.name} is required for ${item.name}.`,`Ask for one of the configured ${group.name} options.`,409,{group:group.name,options:group.options.filter(option=>option.available).map(option=>option.name)})}for(const groupId of requested.modifierDeclines||[])if(!groups.has(groupId))throw new AiToolError("INVALID_MODIFIER","A declined modifier group is not assigned to that product.","Use only configured groups.",409);for(const topping of requested.pizzaToppings||[]){const match=options.get(topping.modifierOptionId);if(!match||match.group.presentationBehavior!=="pizza_topping")throw new AiToolError("INVALID_MODIFIER","A topping is not valid for that product.","Do not convert it to a note.",409)}for(const id of [...Object.keys(requested.modifierQuantities||{}),...Object.keys(requested.modifierAmounts||{})])if(!options.has(id))throw new AiToolError("INVALID_MODIFIER","A modifier ID is not valid for that product.","Do not convert it to a note.",409);const combo=requested.comboId?item.combos.find(row=>row.id===requested.comboId):undefined;if(requested.comboId&&!combo)throw new AiToolError("INVALID_MODIFIER","A combo is not assigned to that product.","Use only configured combos.",409);for(const[groupId,optionIds]of Object.entries(requested.comboSelections||{})){const group=combo?.groups.find(row=>row.id===groupId);if(!group||optionIds.some(id=>!group.options.some(option=>option.id===id&&option.available)))throw new AiToolError("INVALID_MODIFIER","A combo option is not assigned to that product.","Use only configured combo choices.",409)}if(String(requested.specialInstructions||"").trim())throw new AiToolError("INVALID_MODIFIER","Free-text notes are not allowed for AI-added products or modifiers.","Use only configured menu options.",409)}}
export async function priceSpokenOrder(input:{business:OrderingBusiness;actor:OrderingActor;service:ServiceType;items:SpokenOrderItem[];orderId?:string|null;callerPhone?:string;firstName?:string;lastName?:string}){
  if(input.items.length&&input.service==="undecided")throw new AiToolError("FULFILLMENT_REQUIRED","Pickup or delivery must be selected before ordering.","Ask whether this is pickup or delivery and wait for the answer.",409);
  const catalog=await indexedSpokenCatalog(input.business),allItems=catalog.flatMap(category=>category.items).filter(item=>item.available);
  const resolved:AiItemInput[]=input.items.map(requested=>{
    if(spokenKey(requested.name)==="frie")throw new AiToolError("INVALID_VARIANT","Fries require a size.","Ask: Small or large?",409,{options:["Small French Fries","Large French Fries"]});
    const spokenName=spokenKey(requested.name),spokenVariant=spokenKey(requested.variant||""),wingPhrase=spokenName.includes("wing"),bonelessRequested=wingPhrase&&(spokenName.includes("boneles")||spokenVariant.includes("boneles")),pizzaPhrase=spokenName.includes("pizza")||["jumbo","large","16 inch","16 in","16"].includes(spokenName),itemQuery=wingPhrase?(bonelessRequested?"Boneless Wings":"Wings"):pizzaPhrase?"Pizza":requested.name,item=strictMatch(itemQuery,allItems,row=>[row.name,...row.aliases,...(itemAliases[spokenKey(row.name)]||[])],"ITEM_NOT_ON_MENU","Menu item"),nameVariant=item.name==="Pizza"?pizzaVariantAlias(spokenName):undefined,subItem=item.variants.some(row=>spokenKey(row.name)==="full sub")&&item.variants.some(row=>spokenKey(row.name)==="1 2 sub"),subVariant=subItem?subVariantAlias(spokenVariant||spokenName,spokenName.includes(" sub")||spokenName.includes("big bos")):undefined,wingCount=[...spokenName.matchAll(/\b(10|12|15|20|24|25|30|40|50)\b/g),...spokenVariant.matchAll(/\b(10|12|15|20|24|25|30|40|50)\b/g)][0]?.[1]||((item.name==="Wings"||item.name==="Boneless Wings")&&Number.isInteger(requested.quantity)&&[10,12,15,20,24,25,30,40,50].includes(Number(requested.quantity))?String(requested.quantity):""),rawVariant=wingCount?`${wingCount} Wings`:subVariant||(spokenVariant&&!/^(bone in|boneles|boneless)$/.test(spokenVariant)?spokenVariant:nameVariant||""),pizzaVariantPrefix=item.name==="Pizza"?pizzaVariantAlias(rawVariant):undefined,variantWanted=rawVariant;
    if(!variantWanted&&item.variants.length>1&&!item.variants.some(row=>row.defaultVariant)){const options=item.variants.filter(row=>row.available!==false).map(row=>row.name),category=spokenKey(item.name);throw new AiToolError("INVALID_VARIANT",category==="pizza"?"What size?":`${item.name} requires a size or option.`,category==="pizza"?"Ask exactly: What size?":"Ask one short size clarification.",409,{options,pendingItem:{category,customerRequest:requested.name,missingRequiredFields:["size"],actualMenuItemId:item.id}})}
    const variant=pizzaVariantPrefix?item.variants.find(row=>spokenKey(row.name).startsWith(pizzaVariantPrefix)):variantWanted?strictMatch(variantWanted,item.variants,row=>[row.name,...row.aliases],"INVALID_VARIANT","Menu variant"):item.variants.find(row=>row.defaultVariant)||item.variants[0];
    if(pizzaVariantPrefix&&!variant)throw new AiToolError("INVALID_VARIANT",`Sorry, ${requested.variant} isn't an available size or option.`,"Ask for a valid pizza size.",409);
    const modifierSelections:Record<string,string[]>={},pizzaToppings:NonNullable<AiItemInput["pizzaToppings"]>=[],spokenModifiers=[...(requested.modifiers||[])],extraWingSauce=(item.name==="Wings"||item.name==="Boneless Wings")&&(/\b(extra sauce|extra saucy|saucy)\b/.test(spokenName)||spokenModifiers.some(value=>/^(extra sauce|extra saucy|saucy)$/.test(spokenKey(value.name))));
    if(extraWingSauce){for(let index=spokenModifiers.length-1;index>=0;index--)if(/^(extra sauce|extra saucy|saucy)$/.test(spokenKey(spokenModifiers[index].name)))spokenModifiers.splice(index,1);const flavor=spokenModifiers.map(value=>spokenKey(value.name)).find(value=>["mild","medium","hot","suicide","bbq","sweet and sassy","sweet and sour","garlic parmesan","open pit bbq"].includes(value))||["garlic parmesan","sweet and sassy","sweet and sour","open pit bbq","suicide","medium","mild","hot","bbq"].find(value=>spokenName.includes(value)),cup=flavor&&({mild:"Mild Sauce (4oz)",medium:"Medium (4oz)",hot:"Hot Sauce (4oz)",suicide:"Suicide Sauce (4oz)",bbq:"BBQ Sauce (4oz)","sweet and sassy":"Sweet & Sassy (4oz)","sweet and sour":"Sweet & Sour (4oz)","garlic parmesan":"Garlic Parmesan (4oz)","open pit bbq":"BBQ Sauce (4oz)"} as Record<string,string>)[flavor];if(cup&&!spokenModifiers.some(value=>spokenKey(value.name)===spokenKey(cup)))spokenModifiers.push({name:cup})}
    if((item.name==="Wings"||item.name==="Boneless Wings")&&!spokenModifiers.length){const sauce=["garlic parmesan","sweet and sassy","sweet and sour","open pit bbq","suicide","medium","mild","hot","bbq","plain"].find(value=>spokenName.includes(value));if(sauce)spokenModifiers.push({name:sauce})}
    for(const requestedModifier of spokenModifiers){const raw=spokenKey(requestedModifier.name),explicitOnItem=/\b(on (it|the|top)|over|melted)\b/i.test(requestedModifier.name),choices=item.modifiers.flatMap(group=>group.options.map(option=>({group,option}))).filter(({option})=>option.available),hasSideNacho=choices.some(({option})=>spokenKey(option.name)==="nacho cheese on side"),key=raw==="nacho cheese"&&!explicitOnItem&&hasSideNacho?"nacho cheese on side":raw,{group,option}=strictMatch(key,choices,row=>[row.option.name,/^blue cheese\s*\(4oz\)$/i.test(row.option.name)?"blue cheese":row.option.name,/^blue cheese\s*\(4oz\)$/i.test(row.option.name)?"blue cheese cup":row.option.name,row.option.name.replace(/\bon side\b/i,"side cup"),row.option.name.replace(/^xtra\b/i,"extra"),row.option.name.replace(/^add\s+/i,"").replace(/^mayonnaise$/i,"mayo")],"INVALID_MODIFIER","Modifier");if(group.presentationBehavior==="pizza_topping")pizzaToppings.push({modifierOptionId:option.id,portion:requestedModifier.portion||"whole",amount:requestedModifier.amount||"regular"});else modifierSelections[group.id]=[...(modifierSelections[group.id]||[]),option.id]}
    const missingRequired=item.modifiers.find(group=>group.presentationContext==="ordinary"&&group.minSelections>0&&(modifierSelections[group.id]?.length||0)<group.minSelections);
    if(missingRequired){const choices=missingRequired.options.filter(option=>option.available).map(option=>option.name),question=spokenKey(missingRequired.name).includes("cheese")?`What cheese: ${choices.join(", ")}?`:`Choose ${missingRequired.name}: ${choices.join(", ")}?`;throw new AiToolError("INVALID_MODIFIER",question,question,409,{group:missingRequired.name,options:choices,pendingItem:{category:spokenKey(item.name),customerRequest:requested.name,actualMenuItemId:item.id,actualVariantId:variant?.id||null,missingRequiredFields:[missingRequired.name]}})}
    const wingCountWasQuantity=Boolean(wingCount)&&Number(requested.quantity)===Number(wingCount)&&(item.name==="Wings"||item.name==="Boneless Wings");
    return{itemId:item.id,variantId:variant?.id||null,quantity:wingCountWasQuantity?1:Math.max(1,Math.trunc(Number(requested.quantity||1))),modifierSelections,pizzaToppings};
  });
