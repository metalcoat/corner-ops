...
function bestMatch<T>(query:string,rows:T[],terms:(row:T)=>string[],kind:string){const wanted=spokenKey(query),ranked=rows.map(row=>({row,score:Math.max(...terms(row).map(term=>similarity(wanted,spokenKey(term))))})).sort((a,b)=>b.score-a.score);if(!ranked[0]||ranked[0].score<.58)throw new AiToolError("NOT_FOUND",`${kind} not found: ${query}.`,`Use the closest menu wording.`,404);if(ranked[1]&&ranked[0].score<.9&&ranked[0].score-ranked[1].score<.04)throw new AiToolError("NOT_FOUND",`${kind} is ambiguous: ${query}.`,`Clarify between the closest menu matches.`,404);return ranked[0].row}
...
async function indexedSpokenCatalog(business:OrderingBusiness){const cached=spokenCatalogCache.get(business);if(cached&&cached.expiresAt>Date.now())return cached.value;const value=await orderingMenuWithVariants(business,"pos");spokenCatalogCache.set(business,{expiresAt:Date.now()+30_000,value});return value}
export async function priceSpokenOrder(input:{business:OrderingBusiness;actor:OrderingActor;service:ServiceType;items:SpokenOrderItem[];orderId?:string|null;callerPhone?:string;firstName?:string;lastName?:string}){
  ...
  const resolved:AiItemInput[]=input.items.map(requested=>{
    const item=bestMatch(requested.name,allItems,row=>[row.name,...(itemAliases[spokenKey(row.name)]||[])],"Menu item"),rawVariant=spokenKey(requested.variant||""),variantWanted=item.name==="Pizza"&&["large","jumbo","jumbo thin"].includes(rawVariant)?"jumbo thin":rawVariant;
    const variant=variantWanted?bestMatch(variantWanted,item.variants,row=>[row.name,...row.aliases],"Menu variant"):item.variants.find(row=>row.defaultVariant)||item.variants[0];
    const modifierSelections:Record<string,string[]>={},pizzaToppings:NonNullable<AiItemInput["pizzaToppings"]>=[];
    for(const requestedModifier of requested.modifiers||[]){const raw=spokenKey(requestedModifier.name),explicitOnItem=/\b(on (it|the|top)|over|melted)\b/i.test(requestedModifier.name),choices=item.modifiers.flatMap(group=>group.options.map(option=>({group,option}))).filter(({option})=>option.available),hasSideNacho=choices.some(({option})=>spokenKey(option.name)==="nacho cheese on side"),key=similarity(raw,"nacho cheese")>.8&&!explicitOnItem&&hasSideNacho?"nacho cheese on side":raw,{group,option}=bestMatch(key,choices,row=>[row.option.name,row.option.name.replace(/\bon side\b/i,"side cup"),row.option.name.replace(/^xtra\b/i,"extra")],"Modifier");if(group.presentationBehavior==="pizza_topping")pizzaToppings.push({modifierOptionId:option.id,portion:requestedModifier.portion||"whole",amount:requestedModifier.amount||"regular"});else modifierSelections[group.id]=[...(modifierSelections[group.id]||[]),option.id]}
