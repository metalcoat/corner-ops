...
export async function priceSpokenOrder(input:{business:OrderingBusiness;actor:OrderingActor;service:ServiceType;items:SpokenOrderItem[];orderId?:string|null;callerPhone?:string;firstName?:string;lastName?:string}){
  ...
  const resolved:AiItemInput[]=input.items.map(requested=>{
    ...
    const spokenName=spokenKey(requested.name),spokenVariant=spokenKey(requested.variant||""),wingPhrase=spokenName.includes("wing"),bonelessRequested=wingPhrase&&(spokenName.includes("boneles")||spokenVariant.includes("boneles")),pizzaPhrase=spokenName.includes("pizza")||["jumbo","large","16 inch","16 in","16"].includes(spokenName),itemQuery=wingPhrase?(bonelessRequested?"Boneless Wings":"Wings"):pizzaPhrase?"Pizza":requested.name,item=strictMatch(itemQuery,allItems,row=>[row.name,...(itemAliases[spokenKey(row.name)]||[])],"ITEM_NOT_ON_MENU","Menu item"),nameVariant=item.name==="Pizza"?pizzaVariantAlias(spokenName):undefined,wingCount=[...spokenName.matchAll(/\b(10|12|15|20|24|25|30|40|50)\b/g),...spokenVariant.matchAll(/\b(10|12|15|20|24|25|30|40|50)\b/g)][0]?.[1]||((item.name==="Wings"||item.name==="Boneless Wings")&&Number.isInteger(requested.quantity)&&[10,12,15,20,24,25,30,40,50].includes(Number(requested.quantity))?String(requested.quantity):""),rawVariant=wingCount?`${wingCount} Wings`:spokenVariant&&!/^(bone in|boneles|boneless)$/.test(spokenVariant)?spokenVariant:nameVariant||"",pizzaVariantPrefix=item.name==="Pizza"?pizzaVariantAlias(rawVariant):undefined,variantWanted=rawVariant;
