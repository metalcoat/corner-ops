...
const itemAliases:Record<string,string[]>={pizza:["pie"],wing:["wing","bone in wings","chicken wings"],"large french frie":["large fry","large fries","large regular fries"],"small french frie":["small fry","small fries"],"buffalo chip":["buffalo chip","chips"]};
...
export async function priceSpokenOrder(input:{business:OrderingBusiness;actor:OrderingActor;service:ServiceType;items:SpokenOrderItem[];orderId?:string|null;callerPhone?:string;firstName?:string;lastName?:string}){
  ...
  const resolved:AiItemInput[]=input.items.map(requested=>{
    ...
    const spokenName=spokenKey(requested.name),pizzaPhrase=spokenName.includes("pizza")||["jumbo","large","16 inch","16 in","16"].includes(spokenName),itemQuery=pizzaPhrase?"Pizza":requested.name,item=strictMatch(itemQuery,allItems,row=>[row.name,...(itemAliases[spokenKey(row.name)]||[])],"ITEM_NOT_ON_MENU","Menu item"),nameVariant=item.name==="Pizza"?pizzaVariantAlias(spokenName):undefined,rawVariant=spokenKey(requested.variant||"")||nameVariant||"",pizzaVariantPrefix=item.name==="Pizza"?pizzaVariantAlias(rawVariant):undefined,variantWanted=rawVariant;
