...
export async function priceSpokenOrder(input:{business:OrderingBusiness;actor:OrderingActor;service:ServiceType;items:SpokenOrderItem[];orderId?:string|null;callerPhone?:string;firstName?:string;lastName?:string}){
  ...
  const resolved:AiItemInput[]=input.items.map(requested=>{
    ...
    const item=strictMatch(requested.name,allItems,row=>[row.name,...(itemAliases[spokenKey(row.name)]||[])],"ITEM_NOT_ON_MENU","Menu item"),rawVariant=spokenKey(requested.variant||""),pizzaVariantPrefix=item.name==="Pizza"?({small:"small",regular:"regular",medium:"regular",large:"jumbo thin",jumbo:"jumbo thin","jumbo thin":"jumbo thin"} as Record<string,string>)[rawVariant]:undefined,variantWanted=rawVariant;
