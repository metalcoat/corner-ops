...
export async function priceSpokenOrder(input:{business:OrderingBusiness;actor:OrderingActor;service:ServiceType;items:SpokenOrderItem[];orderId?:string|null;callerPhone?:string;firstName?:string;lastName?:string}){
  ...
  const resolved:AiItemInput[]=input.items.map(requested=>{
    ...
    const spokenName=spokenKey(requested.name),pizzaPhrase=spokenName.includes("pizza")||["jumbo","large","16 inch","16 in","16"].includes(spokenName),itemQuery=pizzaPhrase?"Pizza":requested.name,item=strictMatch(itemQuery,allItems,row=>[row.name,...(itemAliases[spokenKey(row.name)]||[])],"ITEM_NOT_ON_MENU","Menu item"),nameVariant=item.name==="Pizza"?(spokenName.includes("sheet")?"sheet":spokenName.includes("thick")?"jumbo thick":/(jumbo|large|\b16\b)/.test(spokenName)?"jumbo thin":/(regular|medium|\b14\b)/.test(spokenName)?"regular":/(small|\b12\b)/.test(spokenName)?"small":""):"",rawVariant=spokenKey(requested.variant||"")||nameVariant,pizzaVariantPrefix=item.name==="Pizza"?({small:"small",regular:"regular",medium:"regular",large:"jumbo thin",jumbo:"jumbo thin","jumbo thin":"jumbo thin","jumbo thick":"jumbo thick",sheet:"sheet pizza"} as Record<string,string>)[rawVariant]:undefined,variantWanted=rawVariant;
