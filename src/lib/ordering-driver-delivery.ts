...
export async function ensureDeliveriesForOrders(actor:DriverActor){if(!actor.manager)return;await ensureDriverDeliverySchema();const sql=getSql();await sql`INSERT INTO ordering_delivery_assignments(id,order_id,business,status,assigned_by) SELECT gen_random_uuid(),o.id,o.business,CASE WHEN o.status='ready' THEN 'READY_FOR_DRIVER' ELSE 'ASSIGNED' END,'driver-pwa' FROM ordering_orders o JOIN ordering_order_delivery_addresses a ON a.order_id=o.id WHERE o.business=${actor.business} AND o.service_type IN('delivery','no_contact_delivery') AND o.status NOT IN('draft','completed','cancelled') AND NOT EXISTS(SELECT 1 FROM ordering_delivery_assignments d WHERE d.order_id=o.id AND d.status NOT IN('DELIVERED','RETURNED','CANCELLED'))`;await sql`UPDATE ordering_delivery_assignments d SET status='READY_FOR_DRIVER',updated_at=NOW() FROM ordering_orders o WHERE o.id=d.order_id AND d.business=${actor.business} AND o.status='ready' AND d.status='ASSIGNED'`}
