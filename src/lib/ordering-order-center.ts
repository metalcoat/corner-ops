...
export async function listOrders(input: { business: OrderingBusiness; date?: string; allOpen?: boolean; query?: string }) {
  ...
  return getSql()`
    SELECT o.id,o.display_number,o.status,o.payment_status,o.service_type,o.source,o.order_origin,
      o.total_cents,o.paid_cents,o.amount_due_cents,o.created_at,o.updated_at,o.scheduled_for,
    ...
    WHERE o.business=${input.business}
      AND (CASE WHEN ${openOnly} THEN o.payment_status NOT IN ('paid','refunded') AND o.status <> 'cancelled' ELSE (o.created_at AT TIME ZONE ${timezone})::date=${date}::date END)
    ...
    ORDER BY CASE WHEN o.payment_status IN ('paid','refunded') THEN 1 ELSE 0 END,
