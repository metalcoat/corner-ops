...
async function refreshCheckTotals(orderId: string) {
  const sql = getSql();
  const items = await sql`SELECT id,quantity,line_total_cents FROM ordering_order_items WHERE order_id=${orderId} ORDER BY sort_order,created_at,id`;
  for (const item of items) {
    ...
    `;
    const assignedQuantity = assignments.reduce((sum, row) => sum + Number(row.quantity), 0);
    if (assignedQuantity !== Number(item.quantity)) throw new CheckConflictError("Check assignments do not match the original order quantity.");
    const unit = Math.floor(Number(item.line_total_cents) / Number(item.quantity));
...
export async function ensureInitialCheck(orderId: string, business: OrderingBusiness, actor: OrderingActor) {
  await ensureOrderingAccountSchema();
  return withTransaction(async () => {
    const sql = getSql();
    const order = (await sql`SELECT id,total_cents FROM ordering_orders WHERE id=${orderId} AND business=${business} FOR UPDATE`)[0];
    if (!order) throw new CheckConflictError("Order was not found.");
    let check = (await sql`SELECT * FROM ordering_checks WHERE order_id=${orderId} ORDER BY display_sequence LIMIT 1`)[0];
    if (!check) {
      const id = randomUUID();
      check = (await sql`INSERT INTO ordering_checks(id,business,order_id,display_sequence,total_cents,amount_due_cents,created_by) VALUES(${id},${business},${orderId},1,${order.total_cents},${order.total_cents},${actor.id}) RETURNING *`)[0];
...
export async function splitCheck(input: { orderId: string; business: OrderingBusiness; fromCheckId: string; lines: Array<{ orderItemId: string; quantity: number }>; actor: OrderingActor }) {
  await ensureInitialCheck(input.orderId, input.business, input.actor);
  if (!input.lines.length) throw new CheckConflictError("Select at least one item quantity to split.");
  return withTransaction(async () => {
    const sql = getSql();
    const order = (await sql`SELECT id,paid_cents FROM ordering_orders WHERE id=${input.orderId} AND business=${input.business} FOR UPDATE`)[0];
    if (!order) throw new CheckConflictError("Order was not found.");
    if (Number(order.paid_cents) > 0) throw new CheckConflictError("Checks cannot be rearranged after a tender has been committed.");
    const source = (await sql`SELECT id FROM ordering_checks WHERE id=${input.fromCheckId} AND order_id=${input.orderId} FOR UPDATE`)[0];
    if (!source) throw new CheckConflictError("Source check was not found.");
    const sequence = Number((await sql`SELECT COALESCE(MAX(display_sequence),0)+1 sequence FROM ordering_checks WHERE order_id=${input.orderId}`)[0].sequence);
    ...
    for (const line of input.lines) {
      if (!Number.isSafeInteger(line.quantity) || line.quantity <= 0) throw new CheckConflictError("Split quantities must be positive whole numbers.");
      const assignment = (await sql`SELECT quantity FROM ordering_check_line_assignments WHERE check_id=${input.fromCheckId} AND order_item_id=${line.orderItemId} FOR UPDATE`)[0];
      if (!assignment || Number(assignment.quantity) < line.quantity) throw new CheckConflictError("A split quantity exceeds the quantity available on the source check.");
...
export async function listChecks(orderId: string, business: OrderingBusiness) {
  await ensureOrderingAccountSchema();
  const sql = getSql();
  const checks = await sql`SELECT * FROM ordering_checks WHERE order_id=${orderId} AND business=${business} ORDER BY display_sequence`;
  for (const check of checks) check.lines = await sql`
