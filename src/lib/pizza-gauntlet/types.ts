...
export type PizzaBuild={ticketId:string;size?:PizzaSize;dough:number;sauceOz:number;cheeseOz:number;sauceSpread?:number;cheeseSpread?:number;toppings:Record<string,{count:number;spread:number}>;bakeSeconds:number;slices?:number;boxed:boolean};
