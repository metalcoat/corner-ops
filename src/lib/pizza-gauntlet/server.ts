...
export function ensureGauntletSchema(){
 if(!schema)schema=(async()=>{const sql=getSql();
...
const code=()=>`JUMBO-${randomBytes(2).toString("hex").toUpperCase()}-${randomBytes(2).toString("hex").toUpperCase()}`;
export async function startRun(playerName:string){await ensureGauntletSchema();const id=randomUUID(),token=randomBytes(32).toString("base64url");await getSql()`INSERT INTO pizza_gauntlet_runs(id,token_hash,player_name)VALUES(${id},${digest(token)},${playerName.trim().slice(0,40)||"Anonymous Cook"})`;return{runId:id,token};}
export async function authenticatedRun(runId:string,token:string){await ensureGauntletSchema();return(await getSql()`SELECT * FROM pizza_gauntlet_runs WHERE id=${runId} AND token_hash=${digest(token)} LIMIT 1`)[0]||null;}
export async function checkpointRun(data:RunCheckpoint,token:string){
 ...
 if(data.stage<Number(run.stage)||data.stage>Number(run.stage)+1||data.stage>STAGES.length)throw new Error("Impossible stage progression.");
 const previous=run.stats||{}, s=data.stats;
 if(data.activeSeconds<Number(run.active_seconds)||data.activeSeconds-Number(run.active_seconds)>1800)throw new Error("Impossible elapsed-time checkpoint.");
 if(s.ordersCompleted<Number(previous.ordersCompleted||0)||s.ordersCompleted-Number(previous.ordersCompleted||0)>20||s.pizzasMade<s.ordersCompleted)throw new Error("Impossible order progression.");
 if(!Number.isFinite(s.accuracy)||s.accuracy<0||s.accuracy>100||s.sanity>120||s.reputation>120)throw new Error("Invalid run statistics.");
 const entry={at:new Date().toISOString(),stage:data.stage,sequence:data.sequence,activeSeconds:data.activeSeconds,orders:s.ordersCompleted,accuracy:s.accuracy,profit:s.profit};
...
export async function completeRun(runId:string,token:string){
 const run=await authenticatedRun(runId,token);if(!run)throw new Error("Run credentials are invalid.");if(run.status==="won"){return(await getSql()`SELECT code,prize_type,issued_at,expires_at FROM pizza_gauntlet_rewards WHERE run_id=${runId}`)[0]}
 const stats=run.stats||{};if(Number(run.stage)!==STAGES.length||Number(run.stage_orders)<STAGES.at(-1)!.ordersRequired||Number(stats.ordersCompleted)<STAGES.reduce((n,x)=>n+x.ordersRequired,0))throw new Error("The full campaign is not complete.");
 // Active time plus dense, monotonic checkpoints makes a browser flag alone insufficient.
 if(Number(run.active_seconds)<5400||!Array.isArray(run.checkpoints)||run.checkpoints.length<18)throw new Error("This run does not contain enough validated active play history.");
 const rewardCode=code(),expires=new Date(Date.now()+PRIZE.expiresDays*86400000).toISOString();
