...
export function calculatePromotions(promotions:Promotion[],lines:PromotionLine[],fulfillmentAt:Date,serviceType:string,channel:string):AppliedPromotion[]{
...
export async function applyPromotionsToOrder(orderId:string){
  await ensureOrderingPromotionSchema();const sql=getSql();
  ...
  const applied=calculatePromotions(promotions,lines,new Date(order.fulfillment_at),String(order.service_type),String(order.source));
