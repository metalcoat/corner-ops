...
export function displayPhone(normalized: string): string {
  ...
  return local.length === 10 ? `(${local.slice(0, 3)}) ${local.slice(3, 6)}-${local.slice(6)}` : normalized;
}
...
export async function findPhoneMatches(business: OrderingBusiness, value: string, excludeCustomerId = "") {
...
export async function createCustomer(input: { business: OrderingBusiness; firstName: string; lastName?: string; phone: string; email?: string; notes?: string }) {
  await ensureOrderingCustomerSchema();
  const normalized = normalizeCallerPhone(input.phone);
  if (!/^\+1\d{10}$/.test(normalized)) throw new Error("Enter a valid 10-digit US/Canada phone number.");
  ...
  return withTransaction(async () => {
    ...
    return { customer: (await findCustomers(input.business, normalized))[0], duplicate: false };
  });
}

export async function addCustomerPhone(input: { business: OrderingBusiness; customerId: string; phone: string; label?: string; isPrimary?: boolean; allowShared?: boolean }) {
  await ensureOrderingCustomerSchema();
  const normalized = normalizeCallerPhone(input.phone);
  if (!/^\+1\d{10}$/.test(normalized)) throw new Error("Enter a valid 10-digit US/Canada phone number.");
  return withTransaction(async () => {
    const sql = getSql();
    const customer = await sql`SELECT id FROM ordering_customers WHERE id=${input.customerId} AND business=${input.business} AND active=TRUE AND merged_into_customer_id IS NULL FOR UPDATE`;
    if (!customer[0]) throw new Error("Active customer not found.");
    const existing = await sql`SELECT * FROM ordering_customer_phones WHERE customer_id=${input.customerId} AND normalized_phone=${normalized}`;
    if (existing[0]) return { phone: existing[0], duplicate: false, alreadyAssociated: true };
    const matches = await findPhoneMatches(input.business, normalized, input.customerId);
    if (matches.length && !input.allowShared) return { duplicate: true, matches };
    const primaryRows = await sql`SELECT id FROM ordering_customer_phones WHERE customer_id=${input.customerId} AND is_primary=TRUE`;
    const isPrimary = Boolean(input.isPrimary) || primaryRows.length === 0;
    if (isPrimary) await sql`UPDATE ordering_customer_phones SET is_primary=FALSE,updated_at=NOW() WHERE customer_id=${input.customerId}`;
    const id = randomUUID();
    const rows = await sql`INSERT INTO ordering_customer_phones(id,customer_id,normalized_phone,display_phone,label,is_primary,last_used_at) VALUES(${id},${input.customerId},${normalized},${displayPhone(normalized)},${String(input.label||"Mobile").trim()},${isPrimary},NOW()) RETURNING *`;
    return { phone: rows[0], duplicate: false, shared: matches.length > 0 };
  });
}

export async function addCustomerAddress(input: { business: OrderingBusiness; customerId: string; label?: string; line1: string; line2?: string; city: string; state: string; postalCode: string; standardizedAddress?: string; provider?: string; providerReferenceId?: string; latitude?: number|null; longitude?: number|null; isPrimary?: boolean }) {
  await ensureOrderingCustomerSchema();
  const customer = await getSql()`SELECT id FROM ordering_customers WHERE id=${input.customerId} AND business=${input.business} AND active=TRUE AND merged_into_customer_id IS NULL`;
  if (!customer[0]) throw new Error("Active customer not found.");
  if (!input.line1.trim() || !input.city.trim() || !input.state.trim() || !input.postalCode.trim()) throw new Error("Street, city, state, and postal code are required.");
  return withTransaction(async()=>{const sql=getSql();const line1=input.line1.trim(),line2=String(input.line2||"").trim(),postal=input.postalCode.trim();
    const existing=await sql`SELECT id FROM ordering_customer_addresses WHERE customer_id=${input.customerId} AND active=TRUE AND lower(line1)=lower(${line1}) AND lower(line2)=lower(${line2}) AND postal_code=${postal}`;
    if(existing[0])return String(existing[0].id);
    const primaryRows=await sql`SELECT id FROM ordering_customer_addresses WHERE customer_id=${input.customerId} AND active=TRUE AND is_primary=TRUE`;
    const isPrimary=Boolean(input.isPrimary)||primaryRows.length===0;if(isPrimary)await sql`UPDATE ordering_customer_addresses SET is_primary=FALSE,updated_at=NOW() WHERE customer_id=${input.customerId} AND active=TRUE`;
    const id=randomUUID();await sql`INSERT INTO ordering_customer_addresses (id,customer_id,label,line1,line2,city,state,postal_code,standardized_address,provider,provider_reference_id,latitude,longitude,last_used_at,is_primary) VALUES (${id},${input.customerId},${String(input.label||"Address")},${line1},${line2},${input.city.trim()},${input.state.trim()},${postal},${String(input.standardizedAddress||"").trim()},${String(input.provider||"")},${String(input.providerReferenceId||"")},${input.latitude??null},${input.longitude??null},NOW(),${isPrimary})`;return id;});
}
...
export async function mergeCustomers(input: { business: OrderingBusiness; survivorId: string; mergedId: string; actorId: string }) {
  ...
  return withTransaction(async () => {
    ...
    if (!(await sql`SELECT id FROM ordering_customer_phones WHERE customer_id=${input.survivorId} AND is_primary=TRUE`)[0]) await sql`UPDATE ordering_customer_phones SET is_primary=TRUE,updated_at=NOW() WHERE id=(SELECT id FROM ordering_customer_phones WHERE customer_id=${input.survivorId} ORDER BY last_used_at DESC NULLS LAST,created_at LIMIT 1)`;
    if (!(await sql`SELECT id FROM ordering_customer_addresses WHERE customer_id=${input.survivorId} AND active=TRUE AND is_primary=TRUE`)[0]) await sql`UPDATE ordering_customer_addresses SET is_primary=TRUE,updated_at=NOW() WHERE id=(SELECT id FROM ordering_customer_addresses WHERE customer_id=${input.survivorId} AND active=TRUE ORDER BY last_used_at DESC NULLS LAST,created_at LIMIT 1)`;
    const combinedNotes = [survivor.notes, merged.notes && `Merged from ${merged.display_name}: ${merged.notes}`].filter(Boolean).join("\n");
