...
export async function findCustomers(business: OrderingBusiness, query = "") {
  ...
  return sql`
    SELECT c.id,c.first_name,c.last_name,c.display_name,c.email,c.notes,c.active,c.created_at,c.updated_at,c.last_order_at,
      ...
      COALESCE((SELECT json_agg(a ORDER BY a.last_used_at DESC NULLS LAST,a.created_at DESC) FROM ordering_customer_addresses a WHERE a.customer_id=c.id AND a.active=TRUE),'[]') addresses
    ...
    WHERE c.business=${business} AND c.active=TRUE AND c.merged_into_customer_id IS NULL
      AND (${q}='' OR c.first_name ILIKE ${like} OR c.last_name ILIKE ${like} OR c.display_name ILIKE ${like} OR p.normalized_phone=${phone})
...
export async function addCustomerAddress(input: { business: OrderingBusiness; customerId: string; label?: string; line1: string; line2?: string; city: string; state: string; postalCode: string; standardizedAddress?: string; provider?: string; providerReferenceId?: string; latitude?: number|null; longitude?: number|null }) {
  await ensureOrderingCustomerSchema();
  ...
  const id = randomUUID();
  await getSql()`INSERT INTO ordering_customer_addresses (id,customer_id,label,line1,line2,city,state,postal_code,standardized_address,provider,provider_reference_id,latitude,longitude,last_used_at) VALUES (${id},${input.customerId},${String(input.label||"")},${input.line1.trim()},${String(input.line2||"").trim()},${input.city.trim()},${input.state.trim()},${input.postalCode.trim()},${String(input.standardizedAddress||"").trim()},${String(input.provider||"")},${String(input.providerReferenceId||"")},${input.latitude??null},${input.longitude??null},NOW())`;
  return id;
}
...
export async function mergeCustomers(input: { business: OrderingBusiness; survivorId: string; mergedId: string; actorId: string }) {
  ...
  return withTransaction(async () => {
    ...
    await sql`UPDATE ordering_customer_addresses a SET customer_id=${input.survivorId} WHERE customer_id=${input.mergedId} AND NOT EXISTS (SELECT 1 FROM ordering_customer_addresses s WHERE s.customer_id=${input.survivorId} AND lower(s.line1)=lower(a.line1) AND lower(s.line2)=lower(a.line2) AND s.postal_code=a.postal_code)`;
    ...
    await sql`INSERT INTO ordering_customer_merge_events (id,business,surviving_customer_id,merged_customer_id,actor_id,field_choices) VALUES (${randomUUID()},${input.business},${input.survivorId},${input.mergedId},${input.actorId},${JSON.stringify({name:"survivor",notes:"combined",addresses:"deduplicated"})}::jsonb)`;
