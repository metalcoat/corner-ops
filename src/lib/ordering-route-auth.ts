...
export type OrderingActor = {
  ...
  email?: string;
  name: string;
  type: "employee" | "account" | "web";
  role?: "employee" | "manager" | "owner";
};

export async function orderingActor(
  business: OrderingBusiness,
): Promise<OrderingActor | null> {
  ...
  return {
    ...
    type: "employee",
    role:
      account.role === "Owner" || account.role === "Co-Owner"
        ? "owner"
...
export async function orderingManagerActor(
  business: OrderingBusiness,
): Promise<OrderingActor | Response> {
