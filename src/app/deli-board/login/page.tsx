...
export default function DeliBoardLoginPage() {
  ...
  return <main className="deliBoard signedOut">
    <section style={{ width: "min(92vw, 480px)" }}>
      ...
      <h1>Deli Board</h1>
      <p>Enter a five-digit Corner Deli employee PIN to activate this display. This uses the restricted employee session, not the owner account.</p>
      <form onSubmit={submit} style={{ display: "grid", gap: 12, marginTop: 22 }}>
        <input
          ...
          autoComplete="off"
          autoFocus
          placeholder="5-digit employee PIN"
        ...
        />
        <button disabled={busy || pin.length !== 5} style={{ border: 0, borderRadius: 10, padding: "13px 16px", fontSize: "1rem", fontWeight: 900, background: "#f59e0b", color: "#111827", cursor: "pointer" }}>{busy ? "Opening board…" : "Open Deli Board"}</button>
      </form>
