...
export default function DeliPosShell({ children, idleLockSeconds, alertSound, alertVolume }: { children: ReactNode; idleLockSeconds: number; alertSound: import("@/lib/ordering-pos-settings").OnlineOrderAlertSound; alertVolume: number }) {
  ...
  useEffect(() => {
    ...
    window.fetch = async (...args) => {
      ...
      const protectedConfiguration = target.startsWith("/api/ordering/settings/") || target.startsWith("/api/ordering/reports") || target.startsWith("/api/ordering/barcodes") || target.startsWith("/api/ordering/gift-cards/report") || target.startsWith("/api/ordering/calls");
  ...
  return <div className="deliPosShell">
    ...
    <header className="deliShellHeader">
      <div className="deliShellUtilities">
        ...
        {session?.authenticated && <Link className={`deliUtilityIcon ${activeWorkspace(pathname, "/pos/deli/ai-calls") ? "active" : ""}`} aria-label="AI call monitor" title="AI call monitor" href="/pos/deli/ai-calls">☎</Link>}
