...
export default function RegisterClient() {
  const [data, setData] = useState<Dashboard | null>(null), [message, setMessage] = useState(""), [amount, setAmount] = useState(""), [reason, setReason] = useState(""), [onesPromptOpen, setOnesPromptOpen] = useState(false), [busy, setBusy] = useState(false), [stationKey, setStationKey] = useState("");
  ...
  async function action(name: string, lowOnOnes?: boolean) {
    setBusy(true); setMessage("");
    try {
      const response = await fetch("/api/ordering/register", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ stationKey, action: name, amountCents: Math.round(Number(amount || 0) * 100), reason, notes: reason, lowOnOnes: name === "close" ? lowOnOnes : undefined }) }), body = await response.json();
      if (!response.ok) throw new Error(body.error || "Register update failed.");
      setAmount(""); setReason(""); setOnesPromptOpen(false); await load();
      setMessage(name === "no_sale" ? "Drawer opened and recorded as No Sale." : name === "close" && body.lowOnOnesAlert?.sent ? "Count saved. Low $1-bill alert text sent." : name === "close" && body.lowOnOnesAlert ? "Count saved, but the low $1-bill text is not configured yet." : name === "close" && body.status === "needs_review" ? "Count saved. A manager must review the drawer variance." : "Register updated.");
  ...
  return <main className="registerPage">
    ...
    {!session ? <section className="registerOpen"><h2>Open register</h2><p>Count the starting cash before taking the first cash payment.</p><label>Opening cash<input type="number" inputMode="decimal" min="0" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} /></label><button disabled={busy} onClick={() => void action("open")}>OPEN REGISTER</button></section> : <>
      ...
      {session.status === "counting" && <section className="registerOpen"><h2>Blind drawer count</h2><p>Enter the physical cash count. The expected amount remains hidden until the count is submitted.</p><label>Counted cash<input autoFocus type="number" inputMode="decimal" min="0" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} /></label><label>Closing note<input value={reason} onChange={e => setReason(e.target.value)} /></label><button disabled={busy} onClick={() => setOnesPromptOpen(true)}>SUBMIT COUNT</button></section>}
    ...
    {onesPromptOpen&&<div className="onesBackdrop" role="presentation"><section className="onesDialog" role="dialog" aria-modal="true" aria-labelledby="ones-question"><h2 id="ones-question">Are there less than $30 in $1 bills?</h2><p>Choosing Yes will send an alert text to the configured owner.</p><div><button type="button" disabled={busy} onClick={()=>setOnesPromptOpen(false)}>BACK</button><button type="button" disabled={busy} onClick={()=>void action("close",false)}>NO</button><button type="button" className="yes" disabled={busy} onClick={()=>void action("close",true)}>YES — SEND ALERT</button></div></section></div>}
  </main>;
}
