...
export default function PosNetworkAccess() {
  ...
  async function check(){const response=await fetch("/api/pos/access/status",{cache:"no-store"}),body=await response.json();setIp(body.ip||"");if(body.allowed){location.replace("/pos/deli");return}setMessage("This network is not approved for POS access.")}
