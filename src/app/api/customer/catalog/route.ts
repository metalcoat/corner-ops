...
export async function GET(request: Request) {
  try {
    ...
    if (!Number.isFinite(at.getTime())) return Response.json({ error: "Invalid menu time." }, { status: 400 });
    const serviceType = ["pickup", "delivery"].includes(params.get("serviceType") || "") ? params.get("serviceType")! : "pickup";
    ...
    const scheduled = await applyScheduledMenuAvailability("Corner Deli", at, raw as unknown as Array<Record<string, any>>);
    await ensureOrderingPromotionSchema();
    const promotions = await getSql()`SELECT customer_label FROM ordering_promotions WHERE business='Corner Deli' AND active=TRUE AND automatic=TRUE AND customer_label<>'' ORDER BY priority DESC,id`;
    const categories = scheduled.map((category: any) => ({
      id: category.id, displayName: category.displayName, parentId: category.parentId, presentationOnly: category.presentationOnly,
      items: category.items.map((item: any) => ({
        id: item.id, categoryId: item.categoryId, displayName: item.name, description: item.description || "", basePriceCents: item.basePriceCents,
        available: Boolean(item.available), imageUrl: item.imageUrl ? item.imageUrl.replace("/api/ordering/media/", "/api/customer/media/") : null, imageAlt: item.imageAlt,
        variants: item.variants.map((variant: any) => ({ id: variant.id, name: variant.name, basePriceCents: variant.basePriceCents, defaultVariant: variant.defaultVariant, available: Boolean(variant.available), modifierPrices: variant.modifierPrices })),
        modifiers: item.modifiers.filter((group: any) => group.presentationContext !== "hidden").map((group: any) => ({ ...group, options: group.options.map((option: any) => ({ id: option.id, name: option.name, priceDeltaCents: option.priceDeltaCents, available: Boolean(option.available), defaultSelected: Boolean(option.defaultSelected), includedQuantity: option.includedQuantity })) })),
        combos: item.combos,
      })),
    }));
