import { employeeMealEligibility, recordEmployeeMeal } from "@/lib/ordering-employee-meals";
import { unauthorized } from "@/lib/http";
import { getPosSession } from "@/lib/pos-auth";

export const runtime = "nodejs";
export async function GET(){const session=await getPosSession(false);if(!session)return unauthorized();const actor={id:session.employeeId,name:session.name,type:"employee" as const,role:session.posRole};return Response.json(await employeeMealEligibility({business:"Corner Deli",actor}));}
