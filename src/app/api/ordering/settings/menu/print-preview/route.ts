...
export async function POST(request: Request) {
  ...
  const modifiers = ids.length ? await sql`SELECT COALESCE(NULLIF(option_print.print_name,''),option.name) name,COALESCE(option_print.print_order,presentation.print_order,0) print_order,COALESCE(presentation.header_modifier,FALSE) header,COALESCE(defaults.default_selected,FALSE) default_selected,COALESCE(option_print.suppress_when_default,presentation.suppress_default_on_ticket,TRUE) suppress_default FROM ordering_modifier_options option JOIN ordering_menu_item_modifier_groups link ON link.group_id=option.group_id AND link.item_id=${String(body.itemId || "")} LEFT JOIN ordering_menu_item_modifier_defaults defaults ON defaults.item_id=link.item_id AND defaults.option_id=option.id AND defaults.active=TRUE LEFT JOIN ordering_modifier_option_print_overrides option_print ON option_print.option_id=option.id LEFT JOIN ordering_modifier_presentation_overrides presentation ON presentation.item_id=link.item_id AND presentation.group_id=link.group_id WHERE option.id=ANY(${ids}::uuid[])` : [];
  const lines = formatKitchenLines([{quantity:1,header:String(item.header),modifiers:modifiers.map((modifier)=>({name:String(modifier.name),printOrder:Number(modifier.print_order),header:Boolean(modifier.header),print:!(modifier.default_selected&&modifier.suppress_default)}))}]);
