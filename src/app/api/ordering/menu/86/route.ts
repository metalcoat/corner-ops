...
export async function POST(request: Request) {
  ...
  const actor = { id: session.employeeId, name: session.name, type: "employee" as const, role: session.posRole };
  try {
    const body = await request.json() as { itemId?: unknown; available?: unknown };
    ...
    const item = await withTransaction(async () => {
      ...
      await sql`UPDATE ordering_menu_items SET available=${available},updated_at=NOW() WHERE id=${itemId}`;
      ...
      await sql`INSERT INTO ordering_operations_audit(id,business,actor_id,actor_role,action,target_type,target_id,reason,details) VALUES(${randomUUID()},'Corner Deli',${actor.id},${actor.role || "employee"},${available?'menu_item_reactivated':'menu_item_86ed'},'item',${itemId},${available?'returned from POS':'86ed from POS'},${JSON.stringify({ itemName: row.name, actorName: actor.name, previouslyAvailable: row.available })}::jsonb)`;
      return { id: String(row.id), name: String(row.name) };
    });
    const email = await sendTransactionalEmail({
      to: [...new Set([...ownerNotificationEmails(), "crfrary@gmail.com"])],
      subject: available ? `Corner Deli item returned: ${item.name}` : `Corner Deli item 86'd: ${item.name}`,
