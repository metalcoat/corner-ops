...
export async function GET(request:Request){try{if(!await orderingActor("Corner Deli"))return unauthorized();const p=new URL(request.url).searchParams;const [orders,settings]=await Promise.all([listOrders({business:"Corner Deli",date:p.get("date")||undefined,allOpen:p.get("view")==="open",query:p.get("q")||""}),getPosSettings("Corner Deli")]);return Response.json({orders,businessTimezone:settings.businessTimezone})}catch(error){return apiError(error)}}
