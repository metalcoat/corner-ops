...
export async function POST(request:Request){
  ...
  const call=(await getSql()`SELECT id,state,order_id,operating_mode FROM ordering_call_sessions WHERE business='Corner Deli' AND three_cx_call_id=${callId} AND state IN('ai','handoff_pending') LIMIT 1`)[0];
  ...
  if(name==="send"&&call.operating_mode!=="autonomous")return reply(rpc.id,{content:[{type:"text",text:JSON.stringify({status:"ORDER_REVIEW_PENDING",mode:call.operating_mode,orderId:String(args.orderId||call.order_id||""),message:"The confirmed draft is visible to Corner Deli staff for review. It has not been accepted by the kitchen."})}]});
