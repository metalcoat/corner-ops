...
function reportType(fileName: string): "shifts" | "orders" | "transactions" | undefined {
  ...
  if (lower.includes("labor") || lower.includes("shift") || lower.includes("attestation")) {
    return "shifts";
  }
...
async function downloadAndImport(
  ...
  fileName: string,
  importedBy: string,
): Promise<{ fileName: string; reportType: string; rowsRead: number; imported: number }> {
  const download = await fetch(url, {
    redirect: "follow",
    cache: "no-store",
  });
  if (!download.ok) {
    throw new Error(`Rezku download failed for ${fileName}: HTTP ${download.status}`);
  }
...
export async function POST(request: Request) {
  ...
  const { data: emailData, error: emailError } = await resend.emails.receiving.get(
    event.data.email_id,
  );
  if (emailError || !emailData) {
    return Response.json(
      { error: emailError?.message || "Received email could not be retrieved." },
      { status: 502 },
    );
  }
  ...
  const allowedSender = (
    process.env.REZKU_ALLOWED_SENDER?.trim() || "support@rezku.com"
  ).toLowerCase();
  const sender = (email.from || event.data.from || "").toLowerCase();
  const subject = email.subject || event.data.subject || "";

  if (sender !== allowedSender || subject !== REZKU_SUBJECT) {
    return Response.json({
      ignored: true,
      reason: "Email did not match the trusted Rezku sender and subject.",
    });
  }

  const sources = excelLinks(`${email.html || ""}\n${email.text || ""}`);
  const { data: rawAttachments, error: attachmentError } =
  ...
  if (uniqueSources.length === 0) {
    return Response.json(
      { error: "The Rezku email did not contain any Excel report links." },
      { status: 422 },
    );
  }
  ...
  for (const source of uniqueSources) {
    results.push(
      await downloadAndImport(
        source.url,
        source.fileName,
        `Resend inbound ${event.data.email_id}`,
      ),
    );
  }
