...
export default function EmployeesPage() {
  ...
  return <main className="workforceShell">
    <header className="workforceHero"><div><p className="wfEyebrow">Staff access, identity, and payroll role</p><h1>Employees</h1><p>Employee colors and photos appear on schedules and messages. Corner Deli positions are limited to Pizza, Fryer, and Delivery; Delivery automatically uses the driver payroll role.</p></div><div className="wfBusinessSwitch">{(["Corner Deli", "Tiki"] as Business[]).map((name) => <button key={name} className={business === name ? "selected" : ""} onClick={() => setBusiness(name)}>{name}</button>)}</div></header>
    {notice && <div className="wfNotice">{notice}</div>}
    <section className="wfTwoColumn">
      <article className="workforcePanel"><div className="wfPanelHeader"><div><p className="wfEyebrow">Add staff</p><h2>New {business} employee</h2></div></div><form className="wfForm" onSubmit={createEmployee}><label>Name<input name="name" required /></label><label>Email<input name="email" type="email" /></label><label>{pinLabel}<input name="pin" inputMode="numeric" pattern={employeePinPattern(business)} minLength={pinLength} maxLength={pinLength} required /></label><label>Position{business === "Corner Deli" ? <select name="position" defaultValue="Pizza" required>{CORNER_DELI_POSITIONS.map((position) => <option key={position}>{position}</option>)}</select> : <input name="position" defaultValue="Bartender" required />}</label>{business !== "Corner Deli" && <label>Role group<select name="roleGroup" defaultValue="In-House"><option>In-House</option><option>Driver</option><option>Ignore</option></select></label>}<label>Hourly rate<input name="hourlyRate" type="number" min="0" step="0.01" defaultValue="0" /></label><label>Tipped rate<input name="tippedRate" type="number" min="0" step="0.01" defaultValue="0" /></label><label className="wfWide"><input name="countsForTips" type="checkbox" defaultChecked /> Include in eligible tip pools</label><button className="wfPrimary" disabled={busy}>Create employee</button></form></article>
      ...
      <article className="workforcePanel employeeDirectoryPanel"><div className="wfPanelHeader"><div><p className="wfEyebrow">Current records</p><h2>{business} employees</h2></div><a className="wfTextLink" href="/ops/workforce">Open scheduler</a></div><div className="employeeDirectoryGrid">{employees.map((employee) => <article className="employeeIdentityCard" key={employee.id} style={{ "--employee-color": employee.scheduleColor } as CSSProperties}><header><span className="employeeIdentityAvatar">{employee.avatarSet ? <img src={avatarUrl(business, employee.id)} alt={`${employee.name} profile`} loading="lazy" /> : initials(employee.name)}</span><div><strong>{employee.name}</strong><span>{employee.position} · {employee.roleGroup}</span><small>{employee.email || (business === "Corner Deli" ? "Imported from Rezku · email not set" : "Email not set")}</small></div></header><div className="employeeIdentityControls"><label>Schedule color<input type="color" value={employee.scheduleColor} disabled={busy} onChange={(event) => void action({ action: "update-profile", id: employee.id, scheduleColor: event.target.value }, `${employee.name}'s schedule color was updated.`)} /></label><label>Position{business === "Corner Deli" ? <select value={employee.position} disabled={busy} onChange={(event) => void action({ action: "update-profile", id: employee.id, position: event.target.value }, `${employee.name}'s position was updated.`)}>{CORNER_DELI_POSITIONS.map((position) => <option key={position}>{position}</option>)}</select> : <input value={employee.position} disabled={busy} onChange={() => undefined} onBlur={(event) => { const position = event.currentTarget.value.trim(); if (position && position !== employee.position) void action({ action: "update-profile", id: employee.id, position }, `${employee.name}'s position was updated.`); }} />}</label><form className="employeePhotoForm" onSubmit={(event) => void uploadPhoto(event, employee)}><label>Icon photo<input name="photo" type="file" accept="image/*" required /></label><button disabled={busy}>Upload photo</button></form></div><footer><span className={`wfBadge ${employee.active ? "approved" : "cancelled"}`}>{employee.active ? "Active" : "Inactive"}</span><span className={`wfBadge ${employee.pinEnabled ? "approved" : "pending"}`}>{employee.pinEnabled ? "PIN set" : "PIN required"}</span><button disabled={busy} onClick={() => editName(employee)}>Edit name</button><button disabled={busy} onClick={() => void action({ action: "update-access", id: employee.id, active: !employee.active }, employee.active ? "Employee deactivated." : "Employee reactivated.")}>{employee.active ? "Deactivate" : "Reactivate"}</button><button disabled={busy} onClick={() => { const pin = window.prompt(`Enter a new ${pinLength}-digit PIN for ${employee.name}`); if (pin) void action({ action: "update-access", id: employee.id, pin }, "PIN updated and Employee Hub access enabled."); }}>Change PIN</button><button disabled={busy} onClick={() => { const email = window.prompt(`Enter an email for ${employee.name}`, employee.email); if (email !== null) void action({ action: "update-access", id: employee.id, email }, "Employee email updated."); }}>Edit email</button></footer></article>)}{employees.length === 0 && <p className="wfEmpty">No employee records for this location yet.</p>}</div></article>
    </section>
  </main>;
}
