...
export default function EmployeesPage() {
  ...
  return <main className="workforceShell">
    <header className="workforceHero"><div><p className="wfEyebrow">Staff access and payroll identity</p><h1>Employees</h1><p>Tiki employees are maintained here. Corner Deli employees are created automatically from Rezku shift reports, then receive Employee Hub access after you assign a five-digit PIN. Names can be corrected at any time and the matching time records follow the correction.</p></div><div className="wfBusinessSwitch">{(["Corner Deli", "Tiki"] as Business[]).map((name) => <button key={name} className={business === name ? "selected" : ""} onClick={() => setBusiness(name)}>{name}</button>)}</div></header>
