...
export default function AttendancePage() {
  ...
  return <main className="controlPage">
    <header className="controlHeader"><div><p className="eyebrow">Attendance exceptions</p><h1>{business} missed shifts</h1><p>Employees receive a secure Employee Hub link and submit exact corrected times there. Nothing reaches payroll until you approve it.</p></div><div className="controlActions"><div className="businessPills">{allowed.map((name) => <button key={name} className={business === name ? "active" : ""} onClick={() => setBusiness(name)}>{name}</button>)}</div><button disabled={busy} onClick={() => void load()}>Refresh</button><a href="/ops/workforce">Workforce</a></div></header>
