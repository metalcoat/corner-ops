...
import { formatUsdFixed } from "@/app/client-format";
...
function CashForecastChart({ weeks, minimum }: { weeks: ForecastWeek[]; minimum: number }) {
  ...
  return <div className="foChartScroll"><svg className="foForecastChart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Thirteen week ending cash forecast">
    {[0, .25, .5, .75, 1].map((ratio) => {
      ...
      return <g key={ratio}><line x1={left} x2={width - right} y1={y(value)} y2={y(value)} className="foGridLine" /><text x={left - 8} y={y(value) + 4} textAnchor="end" className="foAxisText">{formatUsdFixed(value)}</text></g>;
    })}
    ...
    {weeks.map((week, index) => <g key={week.weekEnd}><circle cx={left + index * step} cy={y(week.endingCash)} r="5" className={week.belowMinimum ? "foCashPoint danger" : "foCashPoint"}><title>{`${week.weekEnd}: ${formatUsdFixed(week.endingCash, 2)}`}</title></circle><text x={left + index * step} y={height - 17} textAnchor="middle" className="foAxisText">{new Date(`${week.weekEnd}T12:00:00`).toLocaleDateString([], { month: "short", day: "numeric" })}</text></g>)}
  </svg></div>;
}
  ...
  return <main className="controlPage foPage">
    ...
    {data && tab === "briefing" && <section className="foSection" id="briefing">
      <div className="foMetricGrid six">
        <Metric label="Connected cash" value={formatUsdFixed(data.briefing.headline.currentCash)} />
        <Metric label="13-week ending cash" value={formatUsdFixed(data.briefing.headline.forecastEndingCash)} tone={data.briefing.headline.forecastEndingCash < minimumCash ? "danger" : "positive"} />
        <Metric label="Open bills" value={formatUsdFixed(data.briefing.headline.openBills)} />
        <Metric label="Net income" value={formatUsdFixed(data.briefing.headline.netIncome)} tone={data.briefing.headline.netIncome >= 0 ? "positive" : "danger"} />
        <Metric label="Labor cost" value={percent(data.briefing.headline.laborCostPercent)} />
        <Metric label="Product contribution" value={formatUsdFixed(data.briefing.headline.contribution)} />
      </div>
    ...
    {data && tab === "forecast" && <section className="foSection" id="forecast">
      ...
      <div className="foMetricGrid four"><Metric label="Current cash" value={formatUsdFixed(data.forecast.summary.currentCash)} /><Metric label="Ending cash" value={formatUsdFixed(data.forecast.summary.endingCash)} tone={data.forecast.summary.endingCash >= minimumCash ? "positive" : "danger"} /><Metric label="Lowest point" value={formatUsdFixed(data.forecast.summary.lowestCash)} detail={`Week ending ${data.forecast.summary.lowestWeek}`} /><Metric label="Known bills + payroll" value={formatUsdFixed(data.forecast.summary.openBillsInForecast + data.forecast.summary.payrollInForecast)} /></div>
    ...
    {data && tab === "inventory" && <section className="foSection" id="inventory">
      ...
      <article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Price intelligence</p><h2>Ingredient and supply costs</h2></div></header><div className="foTableWrap"><table><thead><tr><th>Item</th><th>On hand / reorder</th><th>Latest price</th><th>Change</th><th>Best recent vendor</th><th>Action</th></tr></thead><tbody>{data.inventory.items.map((item) => <tr className={item.needsReorder ? "dangerRow" : ""} key={item.id}><td><strong>{item.name}</strong><small>{item.category} · per {item.baseUnit}</small></td><td>{number(item.currentQuantity, 2)} / {number(item.reorderPoint, 2)}</td><td>{item.latestPrice === null ? "No purchase data" : `${formatUsdFixed(item.latestPrice, 4)} · ${item.latestVendor}`}</td><td className={(item.priceChangePercent || 0) > 0 ? "negativeText" : "positiveText"}>{item.priceChangePercent === null ? "—" : `${item.priceChangePercent > 0 ? "+" : ""}${item.priceChangePercent}%`}</td><td>{item.bestRecentVendor ? `${item.bestRecentVendor} · ${formatUsdFixed(item.bestRecentPrice || 0, 4)}` : "—"}</td><td><button onClick={() => { const value = window.prompt(`Current quantity for ${item.name}`, String(item.currentQuantity)); if (value !== null) void post({ action: "adjust-inventory", inventoryItemId: item.id, currentQuantity: value }, "Inventory quantity updated."); }}>Count</button></td></tr>)}</tbody></table></div></article>
      <section className="foRecipeGrid">{data.inventory.recipes.map((recipe) => <article className={`controlCard foRecipe ${recipe.complete ? "" : "incomplete"}`} key={recipe.id}><header><div><p className="eyebrow">{recipe.complete ? "Costed recipe" : "Needs cost data"}</p><h2>{recipe.productName}</h2></div><strong>{recipe.unitCost === null ? "—" : formatUsdFixed(recipe.unitCost, 2)}</strong></header><div className="foRecipeMetrics"><span>Selling price <b>{formatUsdFixed(recipe.sellingPrice, 2)}</b></span><span>Food cost <b>{percent(recipe.foodCostPercent)}</b></span><span>Margin <b>{recipe.contributionMargin === null ? "—" : formatUsdFixed(recipe.contributionMargin, 2)}</b></span><span>30% target price <b>{recipe.recommendedPriceAt30Percent === null ? "—" : formatUsdFixed(recipe.recommendedPriceAt30Percent, 2)}</b></span></div><div className="foCompactList">{recipe.components.map((component) => <div key={component.id}><div><strong>{component.itemName}</strong><span>{number(component.quantity, 4)} {component.unit} · {component.wastePercent}% waste</span></div><b>{component.cost === null ? "Unit mismatch / no price" : formatUsdFixed(component.cost, 2)}</b><button onClick={() => void post({ action: "remove-recipe-component", recipeId: recipe.id, componentId: component.id }, "Recipe component removed.")}>×</button></div>)}{!recipe.components.length && <p>No ingredients added.</p>}</div></article>)}</section>
    </section>}

    {data && tab === "labor" && <section className="foSection" id="labor">
      <div className="foMetricGrid five"><Metric label="Sales" value={formatUsdFixed(data.labor.summary.totalSales)} /><Metric label="Labor hours" value={`${number(data.labor.summary.totalLaborHours)}h`} /><Metric label="Estimated labor cost" value={formatUsdFixed(data.labor.summary.totalLaborCost)} /><Metric label="Sales per labor hour" value={formatUsdFixed(data.labor.summary.salesPerLaborHour)} /><Metric label="Labor cost %" value={percent(data.labor.summary.laborCostPercent)} tone={(data.labor.summary.laborCostPercent || 0) > 35 ? "danger" : ""} /></div>
      <div className="foTwoColumn"><article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Staffing pattern</p><h2>Productivity by weekday</h2></div></header><div className="foTableWrap"><table><thead><tr><th>Day</th><th>Avg sales</th><th>Avg hours</th><th>Sales / hour</th><th>Orders / hour</th><th>Labor %</th></tr></thead><tbody>{data.labor.weekdays.map((row) => <tr key={row.weekday}><td>{row.weekday}<small>{row.days} observed</small></td><td>{formatUsdFixed(row.averageSales)}</td><td>{number(row.averageLaborHours)}</td><td>{formatUsdFixed(row.salesPerLaborHour)}</td><td>{number(row.ordersPerLaborHour, 2)}</td><td>{percent(row.laborCostPercent)}</td></tr>)}</tbody></table></div></article><article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Exceptions</p><h2>Days to review</h2></div></header><div className="foActionList">{data.labor.exceptions.map((row) => <div className="foAction warning" key={row.date}><span>{row.weekday}</span><div><strong>{new Date(`${row.date}T12:00:00`).toLocaleDateString()}</strong><p>{row.reason}</p></div><b>{formatUsdFixed(row.salesPerLaborHour)}/hr</b></div>)}{!data.labor.exceptions.length && <div className="foEmpty">No material low-productivity days in the selected period.</div>}</div></article></div>
      <article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Daily detail</p><h2>Sales and labor</h2></div></header><div className="foTableWrap"><table><thead><tr><th>Date</th><th>Sales</th><th>Orders</th><th>Hours</th><th>Labor cost</th><th>Sales / hour</th><th>Labor %</th></tr></thead><tbody>{data.labor.daily.map((row) => <tr key={row.date}><td>{row.date}</td><td>{formatUsdFixed(row.sales)}</td><td>{row.orders}</td><td>{number(row.laborHours)}</td><td>{formatUsdFixed(row.laborCost)}</td><td>{formatUsdFixed(row.salesPerLaborHour)}</td><td>{percent(row.laborCostPercent)}</td></tr>)}</tbody></table></div></article>
    </section>}

    {data && tab === "profitability" && <section className="foSection" id="profitability">
      <div className="foMetricGrid four"><Metric label="Product sales" value={formatUsdFixed(data.profitability.summary.sales)} /><Metric label="Estimated ingredient cost" value={formatUsdFixed(data.profitability.summary.estimatedCost)} /><Metric label="Contribution margin" value={formatUsdFixed(data.profitability.summary.contribution)} /><Metric label="Complete recipe coverage" value={`${data.profitability.summary.recipeCoveragePercent}%`} tone={data.profitability.summary.recipeCoveragePercent < 80 ? "danger" : "positive"} /></div>
      <div className="foTwoColumn"><article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Dayparts</p><h2>Business by time of day</h2><span>{data.profitability.daypartCoverage === "Orders only" ? "Corner Deli product exports do not link item sales to order time, so dayparts show order volume only." : "Sales and order volume by local order time."}</span></div></header><div className="foDayparts">{data.profitability.dayparts.map((row) => <div key={row.daypart}><span>{row.daypart}</span><strong>{data.profitability.daypartCoverage === "Sales" ? formatUsdFixed(row.sales) : `${row.orders} orders`}</strong><small>{row.orders} orders</small></div>)}</div></article><article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Coverage warning</p><h2>Margin confidence</h2></div></header><p className="foLargeText">Only products with complete recipe components and current ingredient prices receive a calculated contribution margin. Products without cost coverage remain visible instead of being assigned a fictional zero cost, because imaginary margins are how menus become expensive hobbies.</p></article></div>
      <article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Product economics</p><h2>Sales, cost, and contribution</h2></div></header><div className="foTableWrap"><table><thead><tr><th>Product</th><th>Qty</th><th>Sales</th><th>Avg price</th><th>Recipe cost</th><th>Contribution</th><th>Margin</th><th>Coverage</th></tr></thead><tbody>{data.profitability.products.map((row) => <tr key={row.product}><td><strong>{row.product}</strong></td><td>{number(row.quantity, 2)}</td><td>{formatUsdFixed(row.sales)}</td><td>{formatUsdFixed(row.averagePrice, 2)}</td><td>{row.recipeCost === null ? "—" : formatUsdFixed(row.recipeCost, 2)}</td><td>{row.contribution === null ? "—" : formatUsdFixed(row.contribution)}</td><td>{row.marginPercent === null ? "—" : `${row.marginPercent}%`}</td><td><span className={`foBadge ${row.costCoverage === "Complete" ? "good" : "warning"}`}>{row.costCoverage}</span></td></tr>)}</tbody></table></div></article>
    </section>}

    {data && tab === "statements" && <section className="foSection" id="statements">
      <div className="foMetricGrid four"><Metric label="Revenue" value={formatUsdFixed(data.statements.profitAndLoss.totalRevenue)} detail={`Prior: ${formatUsdFixed(data.statements.profitAndLoss.priorRevenue)}`} /><Metric label="Expenses" value={formatUsdFixed(data.statements.profitAndLoss.totalExpenses)} detail={`Prior: ${formatUsdFixed(data.statements.profitAndLoss.priorExpenses)}`} /><Metric label="Net income" value={formatUsdFixed(data.statements.profitAndLoss.netIncome)} tone={data.statements.profitAndLoss.netIncome >= 0 ? "positive" : "danger"} /><Metric label="Balance check" value={data.statements.balanceSheet.balanced ? "Balanced" : formatUsdFixed(data.statements.balanceSheet.balanceDifference)} tone={data.statements.balanceSheet.balanced ? "positive" : "danger"} /></div>
      ...
      <div className="foTwoColumn"><article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Direct method</p><h2>Cash movement</h2><span>{data.statements.cashFlow.method}</span></div></header><div className="foMetricGrid three"><Metric label="Cash in" value={formatUsdFixed(data.statements.cashFlow.cashIn)} /><Metric label="Cash out" value={formatUsdFixed(data.statements.cashFlow.cashOut)} /><Metric label="Net cash" value={formatUsdFixed(data.statements.cashFlow.netCash)} tone={data.statements.cashFlow.netCash >= 0 ? "positive" : "danger"} /></div><div className="foTableWrap"><table><thead><tr><th>Month</th><th>Cash in</th><th>Cash out</th><th>Net</th></tr></thead><tbody>{data.statements.cashFlow.directMonthly.map((row) => <tr key={row.month}><td>{row.month}</td><td>{formatUsdFixed(row.cashIn)}</td><td>{formatUsdFixed(row.cashOut)}</td><td className={classForAmount(row.netCash)}>{formatUsdFixed(row.netCash)}</td></tr>)}</tbody></table></div></article><article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Ledger integrity</p><h2>Trial balance</h2></div></header><div className="foTableWrap"><table><thead><tr><th>Account</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead><tbody>{data.statements.trialBalance.accounts.map((row) => <tr key={row.code}><td>{row.code} · {row.name}</td><td>{formatUsdFixed(row.endingDebit, 2)}</td><td>{formatUsdFixed(row.endingCredit, 2)}</td><td>{formatUsdFixed(row.endingBalance, 2)}</td></tr>)}</tbody><tfoot><tr><th>Total</th><th>{formatUsdFixed(data.statements.trialBalance.totalDebits, 2)}</th><th>{formatUsdFixed(data.statements.trialBalance.totalCredits, 2)}</th><th>{formatUsdFixed(data.statements.trialBalance.totalDebits - data.statements.trialBalance.totalCredits, 2)}</th></tr></tfoot></table></div></article></div>
      <article className="controlCard"><header className="foPanelHeader"><div><p className="eyebrow">Drill-down</p><h2>Journal entries</h2></div></header><div className="foTableWrap"><table><thead><tr><th>Date</th><th>Description</th><th>Source</th><th>Reference</th><th>Debit</th><th>Credit</th></tr></thead><tbody>{data.statements.entrySummary.map((entry) => <tr key={entry.id}><td>{entry.date}</td><td>{entry.description}<small>{entry.lines} lines</small></td><td>{entry.source}</td><td>{entry.reference}</td><td>{formatUsdFixed(entry.debit, 2)}</td><td>{formatUsdFixed(entry.credit, 2)}</td></tr>)}</tbody></table></div></article>
    </section>}
  </main>;
}
...
function StatementPanel({ title, leftTitle, left, rightTitle, right, leftTotal, rightTotal, footerLabel, footerValue }: { title: string; leftTitle: string; left: StatementAccount[]; rightTitle: string; right: StatementAccount[]; leftTotal: number; rightTotal: number; footerLabel: string; footerValue: number }) {
  return <article className="controlCard foStatement"><header className="foPanelHeader"><div><p className="eyebrow">Ledger statement</p><h2>{title}</h2></div></header><div className="foStatementColumns"><div><h3>{leftTitle}</h3>{left.map((row) => <div key={row.code}><span>{row.code} · {row.name}</span><b>{formatUsdFixed(title.startsWith("Balance") ? row.endingBalance : row.periodBalance, 2)}</b></div>)}<footer><strong>Total {leftTitle.toLowerCase()}</strong><b>{formatUsdFixed(leftTotal, 2)}</b></footer></div><div><h3>{rightTitle}</h3>{right.map((row) => <div key={`${row.code}-${row.name}`}><span>{row.code} · {row.name}</span><b>{formatUsdFixed(title.startsWith("Balance") ? row.endingBalance : row.periodBalance, 2)}</b></div>)}<footer><strong>Total {rightTitle.toLowerCase()}</strong><b>{formatUsdFixed(rightTotal, 2)}</b></footer></div></div><div className="foStatementFooter"><strong>{footerLabel}</strong><b className={classForAmount(footerValue)}>{formatUsdFixed(footerValue, 2)}</b></div></article>;
}
